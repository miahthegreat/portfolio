...
export const columns: ColumnDef<PSUData>[] = [
    ...
    },
    {
        ...
        header: ({ column }) => {
            const sortBy = column.getIsSorted();
            return (
              <button
                className="group p-0 m-0 flex items-center gap-2 hover:underline underline-offset-8"
                onClick={() => {
                    column.toggleSorting(sortBy === "asc")
                }}
              >
                <span>Internet PSUs</span>
                <ArrowUpDown className="h-4 w-4 underline-offset-8 hover:underline" />
              </button>
            )
        },
        cell: info => {
